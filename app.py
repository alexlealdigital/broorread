...
# ─────────────────────────────────────────────
# ROTA DE PAGAMENTO COM CARTÃO DE CRÉDITO
# ─────────────────────────────────────────────
@app.route("/api/cobrancas-cartao", methods=["POST"])
def create_cobranca_cartao():
    """Processa pagamento com cartão de crédito via Checkout Transparente do MP."""
    try:
        dados = request.get_json()
        if not dados:
            return jsonify({"status": "error", "message": "Nenhum dado enviado."}), 400

        token           = dados.get("token")           # token do cartão gerado pelo SDK JS
        ...
        payment = payment_response["response"]
        status_mp = payment.get("status")          # approved / in_process / rejected
        ...
        # Se aprovado direto, entrega imediatamente via worker
        ...
        db.session.rollback()
        print(f"[CARTAO] ERRO CRÍTICO: {str(e)}")
